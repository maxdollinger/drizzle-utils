...
export const getSqlString = <T extends Selection>(
  db: DrizzleDB,
  selection: T,
): string => {
  const sqlChunks = Object.entries(selection).map(
...
export const buildExecute =
  (db: DrizzleDB) =>
  async <T extends Selection>(
    selection: T,
    log: boolean = false,
  ): Promise<SelectResultFields<T>[]> => {
    const sqlChunks = Object.entries(selection).map(
      ([key, statement]) => sql`${statement} as "${sql.raw(key)}"`,
    );
    const rawSql =
      sql`select ${sql.join(sqlChunks, sql.raw(","))};`.inlineParams();
...
export const extractValue = async <
