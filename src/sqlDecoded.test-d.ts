...
describe("sqlDecoded", () => {
    ...
    test("function overloading for sql statements with primitives", () => {
        expectTypeOf(sqlDecoded(sql<string>``)).toEqualTypeOf<SQLDecoded<string, string>>();

        expectTypeOf(sqlDecoded(sql<Date>``, (v: string) => new Date(v))).toEqualTypeOf<SQLDecoded<Date, string>>;
