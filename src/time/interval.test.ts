...
import { interval, MS_PER_MINUTE, MS_PER_MONTH, MS_PER_SECOND } from "./interval";
...
describe("pg interval", async () => {
    ...
    test("should return correct ms", async () => {
        const value = await extractValue(exec({ interval: interval({ month: 1, minute: 1, second: 1 }) }), "interval");
